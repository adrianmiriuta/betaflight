...
static void imuMahonyAHRSupdate(float dt, quaternion *vGyro, bool useAcc, quaternion *vAcc, bool useMag, quaternion *vMag, bool useYaw, float yawError) {
...
#endif
    //debug
    DEBUG_SET(DEBUG_IMU, DEBUG_IMU_VACCMODULUS, lrintf(quaternionModulus(vAcc) * 1000));
    if (useAcc) {
    ...
    DEBUG_SET(DEBUG_IMU, DEBUG_IMU_QATTITUDEMODULUS, lrintf(quaternionModulus(&qAttitude) * 1000));
}
